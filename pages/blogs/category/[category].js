  ...
  const fetchData = async () => {
    try {
      let res = await axios.post(
        ...
        {
          category: router.query.category.includes("-")
            ? router.query.category.split("-").join(" ").toUpperCase()
                            ...
                            <Link
                              href={`/blogs/${
                                title && title[title.length - 1] == "?"
                                  ? title.split(" ").join("-").toLowerCase().slice(0, title.length - 1)
                                  : title.split("|").join(" ").split("?").join(" ").split(".").join("").split(" ").join("-").toLowerCase()
                              }`}
                            >
                              Read More
                            </Link>
