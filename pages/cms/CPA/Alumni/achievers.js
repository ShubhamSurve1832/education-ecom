...
const achievers = () => {
    ...
    const postData = async () => {
        ...
        const result = await axios.post(cmsConstats.addCPAAlumniOurAchievers, data, {
            headers: {
    ...
    const fetchData = async () => {
        ...
        setdefaultDeta([...data.cpaAlumniCard1])
    ...
    const editData = async (e) => {
        ...
        const result = await axios.post(cmsConstats.editCPAAlumniOurAchievers, data, {
            headers: {
                'Content-Type': 'multipart/form-data',
                'Authorization': `JWT ${Token}`
            }
    ...
    const deleteListData = async (ind) => {
        ...
        if (confirm("Data Will Be Deleted Permanently.!")) {
            ...
            const result = await axios.post(cmsConstats.deleteCPAAlumniOurAchievers, { type: "cpaAlumniCard1", id: ind }, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': `JWT ${Token}`
                }
                        ...
                        <form action="" method='POST' className=' mt-4' onSubmit={edit ? editData : SubmitData}>
                            ...
                            <div className=" wdt-full bb" style={{ position: 'relative' }}>
                                ...
                                <div className="inputFileds wdt-full bb mt-4" style={{ position: 'relative' }}>
                                    <label htmlFor="" className='title01'> Image rq (Logo Version) :</label>
                                    <h6 className='dimension'>*Dimensions : 60 * 60 | Format : .jpg, .png</h6>

                                    <div className="filedwrap two-col">
                                        <div className="imgBack">
                                            <p>{data.company.name ? data.company.name : ''}</p>
                                            <input type="file" style={{ display: 'none' }} id={`company`} onChange={(e) => handleImageInputChange(e)} className='wdt-full bordernone title01 posiinput' name='company' />
                                        </div>
                                        <div className="one-col right">
                                            <div className="addwrap">
                                                <a className='btn maroon-border-btn text-center' onClick={(e) => clickUploadImage(`company`)}>Add Image</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
export default AdminWithAuth(achievers)