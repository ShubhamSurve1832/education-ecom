...
const unique = () => {
    ...
    const postData = async () => {
        ...
        const result = await axios.post(cmsConstats.addMakeUsUnique, data, {
            headers: {
    ...
    const fetchData = async () => {
        ...
        setdefaultDeta([...data.basicsOfAccounting.helps])
        // console.log(data.basicsOfAccounting.helps);
    }
    ...
    const editData = async (e) => {
        ...
        const result = await axios.post(cmsConstats.editMakeUsUnique, data, {
            headers: {
                'Content-Type': 'multipart/form-data',
                'Authorization': `JWT ${Token}`
            }
    ...
    const deleteListData = async (ind) => {
        ...
        if (confirm("Data Will Be Deleted Permanently.!")) {
            const result = await axios.post(cmsConstats.deleteMakeUsUnique, { id: ind }, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization': `JWT ${Token}`
                }
        ...
        <>
            ...
            <section className="section fixedPadd">
                <div className="container">
                    <h2 className='heading03'>What makes us unique</h2>
                    <form action="" method='POST'>

                        <div>
                            <div className="mt-4">
                                <div className=" bb wdt-full">
                                    <label htmlFor="" className='title01'>Caption</label>
                                    <input ref={blinkOnAddOrEdit} className='wdt-full bordernone title01 inputFileds' placeholder='Caption' name='caption' type="text" value={data.caption} onChange={(e) => handleInputChange(e)} />
                                </div>
                                <div className=" wdt-full bb mt-4" style={{ position: 'relative' }}>
                                    ...
                                    <div className="inputFileds wdt-full">
                                        ...
                                        <input className='wdt-full bordernone title01' placeholder='Alt Text' name='helpAlt' type="text" value={data.helpAlt} onChange={(e) => handleInputChange(e)} />
                                    </div>
                                </div>
                            </div>
                                ...
                                DefaultData && DefaultData.map((data, ind) => {
                                    ...
                                    return (
                                        <tr key={ind}>
                                            ...
                                            <td><p>{data.caption ? data.caption : 'no Alt Available'}</p></td>
                                            <td>
...
export default AdminWithAuth(unique)