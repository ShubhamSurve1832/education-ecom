...
const content = () => {
  ...
  const postData = async () => {
    ...
    const result = await axios.post(cmsConstats.addCPAContentPartners, data, {
      headers: {
  ...
  const fetchData = async () => {
    ...
    setdefaultDeta([...data.cpaPage.repeats])
    // console.log(data.cpaPage.repeats);
  }
  ...
  const editData = async (e) => {
    ...
    const result = await axios.post(cmsConstats.editCPAContentPartners, data, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'Authorization': `JWT ${Token}`
      }
  ...
  const deleteListData = async (ind) => {
    ...
    if (confirm("Data Will Be Deleted Permanently.!")) {
      const result = await axios.post(cmsConstats.deleteCPAContentPartners, { id: ind }, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'Authorization': `JWT ${Token}`
        }
    ...
    <>
      ...
      <section className="section fixedPadd">
        <div className="container">
          <h2 className='heading03'>Our Content Partners </h2>
            ...
            <form action="" method='POST' className='two-col mt-4'>
              <div>
                ...
                <div className="inputFileds bb wdt-full mt-4">
                  ...
                  <input className='wdt-full bordernone title01' name='descriptions' type="text" onChange={(e) => handleInputChange(e)} value={data.descriptions} />
                </div>
                <div className="inputFileds bb wdt-full mt-4">
                  <label htmlFor="" className='title01'>Button Name</label>
                  <input className='wdt-full bordernone title01' name='btnName' type="text" onChange={(e) => handleInputChange(e)} value={data.btnName} />
                </div>
                <div className="inputFileds bb wdt-full mt-4">
                  <label htmlFor="" className='title01'>Button URL</label>
                  <input className='wdt-full bordernone title01' name='btnUrl' type="text" onChange={(e) => handleInputChange(e)} value={data.btnUrl} />
                </div>
              </div>
                ...
                DefaultData && DefaultData.map((data, ind) => {
                  ...
                  return (
                    <tr key={ind}>
                      ...
                      <td><p>{data.para ? data.para : 'no Alt Available'}</p></td>
                      <td>
...
export default AdminWithAuth(content)