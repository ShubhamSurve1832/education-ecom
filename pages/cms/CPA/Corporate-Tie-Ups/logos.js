...
const giant = () => {
	...
	const [data, setData] = useState({ path: {}, imageAlt: "" })
	...
	const postData = async () => {
		...
		const result = await axios.post(cmsConstats.tieUpsAdd, data, {
			headers: {
	...
	const fetchData = async () => {
		...
		setdefaultDeta([...data.CorporateLogos.lists])
	...
	const editData = async (e) => {
		...
		const result = await axios.post(cmsConstats.tieUpsEdit, data, {
			headers: {
				'Content-Type': 'multipart/form-data',
				'Authorization': `JWT ${Token}`
			}
	...
	const deleteListData = async (ind) => {
		...
		if (confirm("Data Will Be Deleted Permanently.!")) {
			const result = await axios.post(cmsConstats.tieUpsDelete, { id: ind }, {
				headers: {
					'Content-Type': 'multipart/form-data',
					'Authorization': `JWT ${Token}`
				}
		...
		<>
			...
			<section className="section fixedPadd">
				<div className="container">
					<h2 className='heading03'>Corporate-Tie-Ups</h2>
					<form action="" method='POST' className='mt-4' onSubmit={edit ? editData : SubmitData}>
							<div className=''>
								<div className="inputFileds wdt-full bb" style={{ position: 'relative' }}>
									...
									<h6 className='dimension'>*Dimensions : 300 * 300 px | Format : .jpg, .png</h6>
								...
								<div className="inputFileds wdt-full mt-4">
									...
									<input ref={blinkOnAddOrEdit} className='wdt-full bordernone title01' name='imageAlt' type="text" onChange={(e) => handleInputChange(e)} value={data.imageAlt} required title='please Provide Alt Text.!' />
								</div>
							</div>
