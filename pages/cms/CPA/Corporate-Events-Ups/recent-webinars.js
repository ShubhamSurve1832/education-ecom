...
const RecentWebinars = () => {
	...
	const postData = async () => {
		...
		const result = await axios.post(cmsConstats.addRecentWebinars, data, {
			headers: {
	...
	const fetchData = async () => {
		...
		setdefaultDeta([...data.allWebinars.lists])
		console.log(data.allWebinars.lists);
	}
	...
	const editData = async (e) => {
		...
		const result = await axios.post(cmsConstats.editRecentWebinars, data, {
			headers: {
				'Content-Type': 'multipart/form-data',
				'Authorization': `JWT ${Token}`
			}
	...
	const deleteListData = async (ind) => {
		...
		if (confirm("Data Will Be Deleted Permanently.!")) {
			...
			const result = await axios.post(cmsConstats.deleteRecentWebinars, { id: ind }, {
				headers: {
					'Content-Type': 'multipart/form-data',
					'Authorization': `JWT ${Token}`
				}
	...
	return (
		<>
			<Head>
				<title>Webinars - Simandhar Education</title>
			</Head>
			<SideTab />
			...
			<section className="section fixedPadd">
				<div className="container">
					<h2 className='heading03'>Recorded Webinars</h2>
						...
						<form action="" method='POST' className='mt-4' onSubmit={edit ? editData : SubmitData}>
							<div className='two-col'>
								...
								<div className="inputFileds bb wdt-full">
									...
									<input className='wdt-full bordernone title01'  name='tag' type="text" value={data.tag} onChange={(e) => handleInputChange(e)} />
								</div>
							</div>
							<div className='two-col mt-4'>
								<div className=" bb wdt-full">
									<label htmlFor="" className='title01'>Video Link (YouTube)</label>
									<input className='wdt-full inputFileds bordernone title01' name='path' type="text" value={data.path} onChange={(e) => handleInputChange(e)} />
								</div>
								...
								<div className="inputFileds wdt-full bb" style={{ position: 'relative' }}>
									...
									<h6 className='dimension'>*Dimensions : 470 * 276 px | Format : .jpg, .png</h6>
									<div className="filedwrap two-col">
										<div className="imgBack">
											<p>{data.thumbnail.name ? data.thumbnail.name : ''}</p>
											<input type="file" style={{ display: 'none' }} onChange={(e) => handleImageInputChange(e)} id={`img`} className='wdt-full bordernone title01 posiinput' name='thumbnail' />
										</div>
							...
							<div className="two-col">
								<div className="two-col">
									<div className="addwrap">
										{edit ? <input type="submit" className='btn maroon-border-btn mt-4 text-center' value="Save" /> : <input type="submit" className='btn maroon-border-btn mt-4 text-center' value="Submit" />}
									</div>
									{/* <div className="addwrap">
						...
						<tbody>
							{
								DefaultData && DefaultData.map((data, ind) => {
									let img = `${S3_buckets}${data.thumbnail}`
									return (
										<tr key={ind}>
											<td><p>{ind + 1}.</p></td>
...
export default AdminWithAuth(RecentWebinars)