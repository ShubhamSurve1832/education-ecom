...
const video = () => {
  ...
  const postData = async () => {
    ...
    const result = await axios.post(cmsConstats.addTopVideo, data, {
      headers: {
  ...
  const fetchData = async () => {
    ...
    let data = await res.json();
      setdefaultDeta([...data.topVideoSection])
    console.log(data);
  }
  ...
  const handleEdit = async(data, ind) => {
    ...
    setedit(true)
    let res = await axios.get(urlConstants.videoSection)
    let videopath = res.data.testimonialVideoPage.videos.filter((val, id) => {
      return val.id === ind;
    })
  ...
  const editData = async (e) => {
    ...
    try{
      const result = await axios.post(cmsConstats.editTopVideo, data, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'Authorization': `JWT ${Token}`
      }
  ...
  // 	const result = await axios.post('https://devapi.simandhareducation.com/cms/deleteCardElements', {id : ind}, { headers: {'Content-Type': 'multipart/form-data',
  // 	'Authorization': `JWT ${Token}`}})
  // 	console.log("result", result);
  // 	console.log(result.data)
    ...
    <>
      ...
      <section className="section fixedPadd">
        <div className="container">
          <h2 className='heading03'>Top video section</h2>
            ...
            <form action="" method='POST' className='mt-4'>
              <div className='two-col'>
                ...
                <div className="inputFileds bb wdt-full">
                  ...
                  <input className='wdt-full bordernone title01' name='videoTag' type="text" value={data.videoTag} onChange={(e) => handleInputChange(e)} />
                </div>
              ...
              <div className="two-col  mt-4">
                <div className=" bb wdt-full">
                  ...
                  <input className='inputFileds wdt-full bordernone title01' name='videoUrl' type="text" value={data.videoUrl} onChange={(e) => handleInputChange(e)} />
                </div>
            ...
            <tbody>
              {
                DefaultData && DefaultData.map((data, ind) => {
                  let img;
                  if (data.videoUrl[0] === '/') {
                    img = `https://simandhareducation-files.s3.ap-south-1.amazonaws.com${data.thumbnail}`
                  } else {
                    img = `https://simandhareducation-files.s3.ap-south-1.amazonaws.com${data.thumbnail}`
                  }
                  return (
