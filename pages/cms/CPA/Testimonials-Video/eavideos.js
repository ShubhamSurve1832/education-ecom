...
const featured = () => {
  ...
  const fetchData = async () => {
    ...
    setdefaultDeta([...data.testimonialTab.testimonialVideoData[2].lists])
    console.log(data.testimonialTab.testimonialVideoData[2].lists);
  }
  ...
  const deleteListData = async (ind) => {
    ...
    if (confirm("Data Will Be Deleted Permanently.!")) {
      const result = await axios.post(cmsConstats.deleteTestimonialVideo, { tab: 2, id: ind }, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'Authorization': `JWT ${Token}`
        }
    ...
    <>
      ...
      <section className="section fixedPadd">
        <div className="container">
          <h2 className='heading03'>EA VIDEOS</h2>
