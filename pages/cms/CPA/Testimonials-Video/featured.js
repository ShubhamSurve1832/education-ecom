...
const featured = () => {
  ...
  const postData = async () => {
    ...
    const result = await axios.post(cmsConstats.addTestimonialVideoPage, data, {
      headers: {
  ...
  const fetchData = async () => {
    ...
    setdefaultDeta([...data.testimonialVideoPage.tabs])
  ...
  const handleEdit = async (data, ind) => {
    ...
    let path = await d.testimonialVideoPage.videos.filter((d) => {
      return d.id == ind
    })
    console.log(path);
  ...
  const editData = async (e) => {
    ...
    const result = await axios.post(cmsConstats.editTestimonialVideoPage, data, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'Authorization': `JWT ${Token}`
      }
  ...
  const deleteListData = async (ind) => {
    ...
    if (confirm("Data Will Be Deleted Permanently.!")) {
      ...
      const result = await axios.post(cmsConstats.deleteTestimonialVideoPage, { id: ind }, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'Authorization': `JWT ${Token}`
        }
    ...
    <>
      ...
      <section className="section fixedPadd">
        <div className="container">
          <h2 className='heading03'>Featured Testimonials</h2>
            ...
            <form action="" method='POST' className='mt-4' onSubmit={edit ? editData : SubmitData}>
              <div className=''>
                ...
                <div className="inputFileds bb wdt-full">
                  ...
                  <input ref={blinkOnAddOrEdit} className='wdt-full bordernone title01' name='heading' type="text" value={data.heading} onChange={(e) => handleInputChange(e)} />
                </div>
                <div className="inputFileds bb wdt-full  mt-4">
                  <label htmlFor="" className='title01'>Video Sub-Title</label>
                  <input className='wdt-full bordernone title01' name='span' type="text" value={data.span} onChange={(e) => handleInputChange(e)} />
                </div>
              </div>
              <div className='mt-4'>
                <div className="inputFileds bb wdt-full">
                ...
                <div className="inputFileds mt-4 wdt-full bb" style={{ position: 'relative' }}>
                  <label htmlFor="" className='title01'> Image rq (Website Version) :</label>
                  <h6 className='dimension'>*Dimensions : 311 * 192 px | Format : .jpg, .png</h6>
                ...
                DefaultData && DefaultData.map((data, ind) => {
                  ...
                  return (
                    <tr key={ind}>
                      ...
                      <td><p>{data.heading ? data.heading : 'No Heading Available'}</p></td>
