...
const featured = () => {
  ...
  const fetchData = async () => {
    ...
    setdefaultDeta([...data.testimonialTab.testimonialVideoData[1].lists])
    console.log(data.testimonialTab.testimonialVideoData[1].lists);
  }
  ...
  const deleteListData = async (ind) => {
    ...
    if (confirm("Data Will Be Deleted Permanently.!")) {
      const result = await axios.post(cmsConstats.deleteTestimonialVideo, { tab: 1, id: ind }, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'Authorization': `JWT ${Token}`
        }
    ...
    <>
      ...
      <section className="section fixedPadd">
        <div className="container">
          <h2 className='heading03'>US CMA VIDEOS</h2>
