...
const journey = () => {
    ...
    const postData = async () => {
        ...
        const result = await axios.post(cmsConstats.addCpaJourney, data, {
            headers: {
    ...
    const fetchData = async () => {
        let res = await fetch(urlConstants.juorneySection)
        let data = await res.json();
        setdefaultDeta([...data.cpaPage.journey])
        // console.log(data.cpaPage.journey);
    }
    ...
    const handleEdit = (data, ind) => {
        ...
        setData({
            ...
            img: {},
            imgAlt: data.imgAlt,
            "descriptions[0]": data.descriptions[0],
            "descriptions[1]": data.descriptions[1],
            "descriptions[2]": data.descriptions[2]
        });
    ...
    const editData = async (e) => {
        ...
        const result = await axios.post(cmsConstats.editCpaJourney, data, {
            headers: {
                'Content-Type': 'multipart/form-data',
               'Authorization': `JWT ${Token}`
            }
    ...
    const deleteListData = async (ind) => {
        ...
        if (confirm("Data Will Be Deleted Permanently.!")) {
            const result = await axios.post(cmsConstats.deleteCpaJourney, { id: ind }, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                   'Authorization': `JWT ${Token}`
                }
        ...
        <>
            ...
            <section className="section fixedPadd">
                <div className="container">
                    <h2 className='heading03'>CPA Journey Simplified</h2>
                        ...
                        <form action="" method='POST' className='two-col mt-4'>
                            <div>
                                ...
                                <div className="inputFileds bb wdt-full mt-4">
                                    <label htmlFor="" className='title01'>Number</label>
                                    <input className='wdt-full bordernone title01' name='number' type="text" onChange={(e) => handleInputChange(e)} value={data.number} />
                                </div>
                                <div className="inputFileds bb wdt-full mt-4">
                                    <label htmlFor="" className='title01'>Short description</label>
                                    <input className='wdt-full bordernone title01' name='descriptions[0]' type="text" onChange={(e) => handleInputChange(e)} value={data['descriptions[0]']} />
                                </div>
                                <div className="inputFileds bb wdt-full mt-4">
                                    <label htmlFor="" className='title01'>Short description</label>
                                    <input className='wdt-full bordernone title01' name='descriptions[1]' type="text" onChange={(e) => handleInputChange(e)} value={data['descriptions[1]']} />
                                </div>
                                <div className="inputFileds bb wdt-full mt-4">
                                    <label htmlFor="" className='title01'>Short description</label>
                                    <input className='wdt-full bordernone title01'  name='descriptions[2]' type="text" onChange={(e) => handleInputChange(e)} value={data['descriptions[2]']} />
                                </div>
                            </div>
                            <div className="wdt-full bb" style={{ position: 'relative' }}>
                                ...
                                <div className="filedwrap two-col mb-4 inputFileds">
                                ...
                                <div className="inputFileds wdt-full">
                                    ...
                                    <input className='wdt-full bordernone title01' name='imgAlt' type="text" onChange={(e) => handleInputChange(e)} value={data.imgAlt} />
                                </div>
                            </div>
