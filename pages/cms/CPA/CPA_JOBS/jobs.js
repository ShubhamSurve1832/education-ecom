...
const jobs = () => {
  ...
  const postData = async () => {
    ...
    const result = await axios.post(cmsConstats.addUSCPAJobs, data, {
      headers: {
  ...
  const fetchData = async () => {
    ...
    setdefaultDeta([...data.cpaJobs.lists])
    // console.log(data.cpaJobs.lists);
  }
  ...
  const editData = async (e) => {
    ...
    const result = await axios.post(cmsConstats.editUSCPAJobs, data, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'Authorization': `JWT ${Token}`
      }
  ...
  const deleteListData = async (ind) => {
    ...
    if (confirm("Data Will Be Deleted Permanently.!")) {
      ...
      const result = await axios.post(cmsConstats.deleteUSCPAJobs, { id: ind }, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'Authorization': `JWT ${Token}`
        }
    ...
    <>
      ...
      <section className="section fixedPadd placement">
        <div className="container">
          <h2 className='heading03'>CPA JOBS</h2>
