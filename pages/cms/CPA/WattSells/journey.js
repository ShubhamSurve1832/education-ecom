...
const journey = () => {
  ...
  const postData = async () => {
    ...
    const result = await axios.post(cmsConstats.addTopperJourney, data, {
      headers: {
  ...
  const fetchData = async () => {
    ...
    setdefaultDeta([...data.topperJourneyData.lists])
    console.log(data.topperJourneyData.lists);
  }
  ...
  const editData = async (e) => {
    ...
    const result = await axios.post(cmsConstats.editTopperJourney, data, {
      headers: {
        'Content-Type': 'multipart/form-data',
         'Authorization': `JWT ${Token}`
      }
  ...
  const deleteListData = async (ind) => {
    ...
    if (confirm("Data Will Be Deleted Permanently.!")) {
      ...
      const result = await axios.post(cmsConstats.deleteTopperJourney, { id: ind }, {
        headers: {
          'Content-Type': 'multipart/form-data',
           'Authorization': `JWT ${Token}`
        }
    ...
    <>
      ...
      <section className="section fixedPadd">
        <div className="container">
          <h2 className='heading03'>Hear What Topper has to Say About His Journey.</h2>
          <div>
            <form action="" method='POST' className='two-col mt-4' onSubmit={edit ? editData : SubmitData}>
              <div>
                <div className="inputFileds bb wdt-full">
                  ...
                  <input ref={blinkOnAddOrEdit} className='wdt-full bordernone title01' placeholder='Video Title' name='title' type="text" value={data.title} onChange={(e) => handleInputChange(e)} />
                </div>
                <div className="inputFileds bb wdt-full mt-4">
                  <label htmlFor="" className='title01'>Video Link (YouTube)</label>
                  <input className='wdt-full bordernone title01' placeholder='Video Link (YouTube)' name='path' type="text" value={data.path} onChange={(e) => handleInputChange(e)} />
                </div>
              </div>
              <div className=" wdt-full bb" style={{ position: 'relative' }}>
                ...
                <div className="filedwrap inputFileds two-col ">
                  <div className="imgBack">
                    <p>{edit ? data.img?.name : data.thumbnail?.name}</p>
                    <input type="file" style={{ display: 'none' }} onChange={(e) => handleImageInputChange(e)} id={`img`} className='wdt-full bordernone title01 posiinput' name={edit ? 'img' : 'thumbnail'} />
                  </div>
