...
const eytakeaways = () => {
	...
	const postData = async () => {
		...
		const result = await axios.post(cmsConstats.addKeyTakeaways, data, {
			headers: {
	...
	const fetchData = async () => {
		...
		setdefaultDeta([...data.eaPage.helps])
		console.log(data.eaPage.helps);
	}
	...
	const handleEdit = (data, ind) => {
		...
		setData({
			id: ind,
			img: {},
			imgAlt: data.imgAlt ? data.imgAlt : data.helpAlt,
	...
	const editData = async (e) => {
		...
		const result = await axios.post(cmsConstats.editKeyTakeaways, data, {
			headers: {
				'Content-Type': 'multipart/form-data',
				'Authorization': `JWT ${Token}`
			}
	...
	const deleteListData = async (ind) => {
		...
		if (confirm("Data Will Be Deleted Permanently.!")) {
			const result = await axios.post(cmsConstats.deleteKeyTakeaways, { id: ind }, {
				headers: {
					'Content-Type': 'multipart/form-data',
					'Authorization': `JWT ${Token}`
				}
		...
		<>
			...
			<section className="section fixedPadd postsSec">
				<div className="container">
					<h2 className='heading03'>Key takeaways</h2>
							...
							<div className=" mt-4">
								...
								<div className="two-col wdt-full bb" style={{ position: 'relative' }}>
									...
									<div className=" bb wdt-full">
										<label htmlFor="" className='title01'>Icon text</label>
										<input ref={blinkOnAddOrEdit} className='wdt-full bordernone title01 inputFileds' placeholder='Icon text' name="caption" onChange={(e) => handleInputChange(e)} type="text" value={data.caption} />
									</div>

									<div className="inputFileds wdt-full">
										<label htmlFor="" className='title01'>Image Alt text :</label>
										<input className='wdt-full bordernone title01' placeholder='Alt Text' name='imgAlt' type="text" onChange={(e) => handleInputChange(e)} value={data.imgAlt} />
									</div><br /><br />
								</div>
							</div>
...
export default AdminWithAuth(eytakeaways)