...
const AccaContent = () => {
    ...
    let fetchData = async () => {
        let res = await axios.get(urlConstant.contentSection)
        setContentData({ ...res.data.cpaPage })
    }
                            ...
                            <div className="content-box p2">
                                ...
                                <div className='download-popup' style={{ display: !isShow1 ? 'none' : 'block' }}>
                                    <span className='close-pp dwn-load' onClick={showDwnPopup1} style={{ display: isShow1 ? 'block' : 'none' }}>
                                        <Image loading='lazy' src="/img/white_cross.png" alt="white cross" fill={true} sizes='100vw' className='resp-img' />
                                    </span>
